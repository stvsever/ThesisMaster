...
# -*- coding: utf-8 -*-
"""
generate_DSM5TR_criteria.py

Generates clinical expression criteria + estimated prevalence for all DSM-5-TR
leaf disorders in a nested JSON.
...
def build_user_query(disorder: str, path: List[str]) -> str:
    ...
    return (
        f"This is the (mental) health disorder: '{disorder}'. "
...
def call_llm_parse(
    ...
    """
    # NOTE: Do NOT pass temperature; some models (e.g., gpt-5-nano) reject non-default temperatures.
...
# ==============================================================================
def main():
    parser = argparse.ArgumentParser(description="Generate DSM-5-TR clinical criteria + estimated prevalence for all leaf disorders.")
    parser.add_argument(
        "--input-json",
        default="/Users/stijnvanseveren/PythonProjects/MASTERPROEF/SystemComponents/PHOENIX_ontology/separate/CRITERION/steps/01_raw/clinical/01_pre_generation/DSM_adjusted/DSM_disorders.json",
        help="Path to input nested DSM JSON.",
    )
    parser.add_argument(
        "--output-dir",
        default="/Users/stijnvanseveren/PythonProjects/MASTERPROEF/SystemComponents/PHOENIX_ontology/separate/CRITERION/steps/01_raw/clinical/02_post_generation/DSM_pg",
    ...
    logger = logging.getLogger("DSM5TR")
    ...
    # System prompt (concise & strict) — left as provided
    system_prompt = (
        ...
        "of a provided (mental) health disorder. Provide a list of observable (internal and/or external) criteria that can be used "
    ...
    merged_outfile = output_dir / f"DSM5TR_criteria_merged_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
