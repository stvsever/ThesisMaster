...
\hypersetup{
  ...
  linkcolor=black,
  urlcolor=black,
  pdftitle={PHOENIX evaluatiestudie - HCP-PRE-03 (C03)},
...
\fancyhead[R]{\small\color{SlateMid}\textbf{HCP-PRE-03}\quad C03}
...
\end{itemize}
\smallskip\small
\textit{De verzamelde EMA-data worden vervolgens ingezet voor \textbf{netwerkanalyse}: het kwantitatief in kaart brengen van de empirische verbanden tussen dagelijkse gedragingen (behandelingsopties) en klachtniveaus (symptomen) over meerdere weken. Het resulterende tweedelig netwerk maakt behandelprioriteiten empirisch zichtbaar (zie Deel 3).}
\end{instrbox}

\vspace{0.4em}
\begin{instrbox}[title={Het tweedelig netwerk: structuur van het observatiemodel}]
\small
Na voldoende EMA-monitoring construeert PHOENIX een \textbf{tweedelig netwerk}:
...
\midrule
 & \textbf{Totaal (1 casus)} & & \textbf{25--35 min} \\
\bottomrule
\end{tabular}
\end{center}
...
Ga pas naar een volgend deel wanneer het huidige deel volledig is afgewerkt. Voor Deel 1--5 geldt dit voor de toegewezen casus; Deel 6 is éénmalig.
...
\begin{responsebox}[title={Uw antwoord -- Deel 1}]
...
% ══════════════════════════════════════════════════════════════════════════════
\section{Deel 2: Initieel observatiemodel (tweedelig netwerk)}
...
\begin{responsebox}[title={Uw antwoord -- Deel 2}]
...
% ══════════════════════════════════════════════════════════════════════════════
\section{Deel 3: Behandeldoelprioritering via het tweedelig netwerk}
...
\textbf{Het tweedelig netwerk lezen:}
\begin{itemize}[topsep=2pt,itemsep=1pt]
...
(gem.\ 65 min). Lichaamsbeweging: 0,8 sessies/week. Ademhaling sessies: gem.\ 2,4/dag (goed).

\medskip
\textbf{Tweedelig netwerk voor dit voorbeeld:}

\begin{center}
\begin{tikzpicture}[node distance=0pt]
  \node[smprnode] (bo1) at (0, 1.3)  {BO-1\\Avondschermgebruik};
  \node[smprnode] (bo2) at (0, 0.0)  {BO-2\\Lichaamsbeweging};
  \node[smprnode] (bo3) at (0,-1.3)  {BO-3\\Ademhaling sessie};
  ...
  \draw[line width=3.5pt, draw=StrongRed!85, opacity=0.90] (bo3.east) -- (s2.west);
  \draw[line width=0.4pt, draw=StrongRed!85, opacity=0.90] (bo3.east) -- (s3.west);
...
\item \textbf{BO-3 (Ademhaling sessie)} -- beschermend effect op S-2 (rode lijn); frequentie hoog (gem.\ 2,4/dag, goed) $\rightarrow$ \textbf{prioriteit 3} (toestand reeds goed).
\end{enumerate}
\end{voorbeeldbox}
...
\begin{tikzpicture}[node distance=0pt]
  ...
  \draw[line width=0.4pt,  draw=StrongRed!85, opacity=0.90] (p1.east) -- (cr4.west);
  \draw[line width=0.4pt,  draw=StrongRed!85, opacity=0.90] (p2.east) -- (cr3.west);
  \draw[line width=0.4pt,  draw=StrongRed!85, opacity=0.90] (p4.east) -- (cr4.west);
...
\begin{responsebox}[title={Uw antwoord -- Deel 3}]
...
\begin{responsebox}[title={Uw antwoord -- Deel 4}]
...
\checkitemBFS{3}{Medicatie op voorgeschreven tijdstip ingenomen (ja/nee)}
\checkitemBFS{4}{Werkgerelateerde gedachten actief afgesloten via vaste afsluiting na werk (ja/nee)}
\checkitemBFS{5}{Gebalanceerde maaltijden op vaste tijden genuttigd vandaag (ja/nee)}
\checkitemBFS{6}{Alcoholconsumptie vanavond (eenheden)}
\checkitemBFS{7}{Omega-3- of vitamine-D-supplement ingenomen (ja/nee)}
\checkitemBFS{8}{Herstelactiviteit buiten het werk actief uitgevoerd (min)}
\checkitemBFS{9}{Sigaretten of tabak gerookt vandaag (aantal)}
\checkitemBFS{10}{Financiele zorgen of administratieve taken aangepakt vandaag (ja/nee)}
...
\checkitemBFS{13}{Vast slaap- en waktijdstip aangehouden vandaag (ja/nee)}
\checkitemBFS{14}{Moment van professionele positieve prestatie bewust opgemerkt (ja/nee)}
\checkitemBFS{15}{Gesprek met vertrouwenspersoon of steunfiguur gevoerd vandaag (ja/nee)}
\checkitemBFS{16}{Bewust dankbaarheidsoefening of positieve reflectie uitgevoerd vandaag (ja/nee)}
\checkitemBFS{17}{Professionele vaardigheid of aanpak bewust en succesvol ingezet (ja/nee)}
\checkitemBFS{18}{Cafeine-inname na 15.00 uur beperkt (ja/nee)}
\checkitemBFS{19}{Professioneel consult of therapeutische sessie bijgewoond vandaag (ja/nee)}
...
\begin{responsebox}[title={Uw antwoord -- Deel 5}]
...
(3)~prioritering op basis van een tweedelig netwerk dat op dagelijkse EMA-data wordt gefit,
...
De kern van deze aanpak is \textbf{symptoomgedreven netwerkanalyse}: klachten worden niet als
...
\begin{tcolorbox}[colback=AccentAmber,colframe=GoldAmber,arc=2mm,boxrule=0.7pt,
  left=10pt,right=10pt,top=8pt,bottom=8pt]
\small\textbf{\color{GoldAmber}Reflectievraag a}\\[0.3em]
Welke informatie zou u via de dagelijkse EMA-vragen willen verzamelen, en hoe zou u
die selectie bepalen op basis van de vrije klachttekst?
\end{tcolorbox}
\begin{responsebox}[title={Uw antwoord -- Reflectievraag a}]
...
\begin{tcolorbox}[colback=AccentAmber,colframe=GoldAmber,arc=2mm,boxrule=0.7pt,
  left=10pt,right=10pt,top=8pt,bottom=8pt]
\small\textbf{\color{GoldAmber}Reflectievraag b}\\[0.3em]
Hoe zou u de dagelijkse boodschappen inhoudelijk afstemmen op wat u via de
EMA-data geleidelijk leert over de toestand en het gedrag van de persoon?
\end{tcolorbox}
\begin{responsebox}[title={Uw antwoord -- Reflectievraag b}]
\writeline\writeline\writeline\writeline\writeline
\end{responsebox}
...
\begin{tcolorbox}[colback=AccentAmber,colframe=GoldAmber,arc=2mm,boxrule=0.7pt,
  left=10pt,right=10pt,top=8pt,bottom=8pt]
\small\textbf{\color{GoldAmber}Reflectievraag c}\\[0.3em]
Zou u uw digitale aanpak systematisch bijsturen naarmate meer data beschikbaar wordt?
Zo ja -- op basis van welke criteria en via welk redeneerproces?
\end{tcolorbox}
\begin{responsebox}[title={Uw antwoord -- Reflectievraag c}]
\writeline\writeline\writeline\writeline\writeline
\end{responsebox}
...
Dank u voor het invullen van alle zes delen voor uw toegewezen casus
en de afsluitende reflectievraag.
