...
\subsection{Background: The PHOENIX System}
...
\smallskip
\textbf{Task:} For each of your \textbf{two assigned case vignettes}, identify and operationalize the key mental health criteria present in the complaint. (This document shows all ten cases for researcher reference; Qualtrics Display Logic will show only your assigned two.)

\textbf{What we mean by ``operationalized criterion'':}
A criterion is a distinct, currently present, changeable dimension of the person's mental state or functioning that (a) reflects a specific complaint aspect, (b) can in principle be measured repeatedly over time (e.g., via daily self-report), and (c) is not a demographic trait or static background factor.
...
\item A \textbf{brief description} (1--2 sentences: what it is and how it manifests here)
\end{itemize}
...
\smallskip
\textbf{Task:} For each case, propose a set of \textbf{modifiable predictor variables} that could be monitored over time (e.g., via daily self-report / EMA) to explain or predict the criteria identified in the standardised context below.

\textbf{What is a modifiable predictor?} A predictor is a behavioural, cognitive, physiological, or environmental variable that (a) can change from day to day, (b) plausibly influences one or more of the criteria, and (c) can be measured by the person themselves with a simple daily rating or count.

\textbf{Response format:} List \textbf{3--5 predictors} using this structured template (one per line):

\begin{center}\small\ttfamily
Label | 1-sentence measurement definition | CR connections (e.g.\ CR-1, CR-3)
\end{center}

This format allows direct automated comparison with machine outputs. Abbreviate freely; clinical precision is more important than length.
...
\smallskip
\textbf{Task:} For each case, \textbf{rank the top 2--4 treatment targets} from the bipartite network diagram provided.

\textbf{What is a treatment target?} A modifiable predictor (P-node) from the initial model that, if changed, is most likely to reduce the person's complaint criteria. Select based on: (a) monitoring data evidence, (b) clinical modifiability, and (c) downstream impact on multiple criteria.

\textbf{Visual key:} Each diagram shows \textbf{\color{PrimaryBlue}criteria nodes (blue, left)} and \textbf{\color{TealGreen}predictor nodes (teal, right)}. Edge weight reflects the monitoring-derived importance of the predictor--criterion relationship:
\begin{center}\small
...
\end{center}
\textbf{Fair comparison note:} The PHOENIX system processes the same numerical edge weights shown here. The visual is a human-readable representation of identical data. No additional information is presented to you vs.\ the machine.

\textbf{Response format:} \textbf{Qualtrics Rank Order item.} Drag/rank the predictor labels from the network in order of treatment priority (1 = highest). Select only 2--4 items.
...
\smallskip
\textbf{Task:} For each case, \textbf{select the 2--4 predictors to retain in the next monitoring cycle}, given the treatment targets identified in Part 3 and the monitoring data pattern.

\textbf{Response format:} \textbf{Multiple Choice (select all that apply)} --- tick the predictors from the initial model that should be tracked next cycle. Optionally add a 1-sentence note per predictor if an operationalization change is needed.

This structured format is directly comparable to the machine output, which produces a binary retain/drop decision per predictor plus an optional rationale string.

\textbf{Context provided:} Standardised treatment targets from Step 03 reference run (shown per case).
...
\smallskip
\textbf{Task:} For each case, complete two items: (A) identify the \textbf{HAPA motivational phase} most relevant to this person, and (B) write a \textbf{brief personalized coaching message}.

\textbf{(A) HAPA phase classification:} Select the phase that best characterizes where this person currently is:
\begin{itemize}[topsep=2pt,itemsep=1pt]
\item \textbf{Pre-intentional} --- not yet motivated to change; needs motivational/risk communication
\item \textbf{Intentional} --- motivated but not yet planning; needs goal-setting and planning support
\item \textbf{Action/Maintenance} --- already attempting change; needs coping planning and self-efficacy support
\end{itemize}

\textbf{(B) Message format:} 2--3 sentences, second-person, warm and actionable. Address the top treatment target and the key coping strategy. No jargon; no diagnostic labels.

\textbf{Response format:} Two Qualtrics items per case --- (A) multiple choice for phase, (B) Text Entry (Medium) for message.
