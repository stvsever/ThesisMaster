...
- Lets you optionally choose a subtree within PERSON.json via --root_key (if your file contains multiple top-level sections)
"""
...
INPUT_JSON = (
    ...
    "PHOENIX_ontology/separate/01_raw/PERSON/PERSON.json"
)

OUTPUT_TXT = (
    "/Users/stijnvanseveren/PythonProjects/MASTERPROEF/utils/official/"
    "ontology_mappings/HAPA/profile_to_barrier/input_lists/"
    "person_factors_list.txt"
)

# Default depth: 3 is a good starting point for many PERSON ontologies
DEFAULT_TARGET_LEVEL = 3

# Default printed root label in the TXT
DEFAULT_ROOT_LABEL = "PERSON"
...
def load_person_subontology(
    person_json: Dict[str, Any],
    *,
    root_key: Optional[str] = None
) -> Dict[str, Any]:
    """
    Returns the dict to traverse from PERSON.json.

    Common shapes supported:
      A) PERSON.json is directly the ontology dict (most common):
         { "Demographics": {...}, "Traits": {...}, ... }

      B) PERSON.json wraps a root key:
         { "PERSON": { "Demographics": {...}, ... } }

      C) PERSON.json contains multiple top-level sections; you want one:
         { "PERSON": {...}, "OTHER": {...} }  or  { "SECTION_A": {...}, "SECTION_B": {...} }
         Use --root_key to pick the section.

    Behavior:
      - If root_key is provided: returns person_json[root_key]
      - Else if "PERSON" exists and is dict: returns person_json["PERSON"]
      - Else returns person_json itself (must be dict)
    """
    if not isinstance(person_json, dict):
        raise TypeError("PERSON.json must contain a JSON object at the top level (dict).")

    if root_key is not None:
        if root_key not in person_json or not isinstance(person_json[root_key], dict):
            raise KeyError(f"Could not find a dict at top-level key '{root_key}' in PERSON.json.")
        return person_json[root_key]

    if "PERSON" in person_json and isinstance(person_json["PERSON"], dict):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create a hierarchical .txt list of nodes from PERSON.json at a chosen depth."
    )
    parser.add_argument(
        "--target_level",
        type=int,
        ...
        help=(
            ...
            "Examples: 2=primary nodes, 3=secondary nodes, 4+=deeper person-factor layers."
        ),
    )
    parser.add_argument(
        "--input_json",
        type=str,
        default=INPUT_JSON,
        help="Path to PERSON.json"
    )
    parser.add_argument(
        "--output_txt",
        type=str,
        default=OUTPUT_TXT,
        help="Path to write person_factors_list.txt"
    )
    parser.add_argument(
        "--root_label",
        type=str,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional: top-level key in PERSON.json to traverse. "
            "Use if your PERSON.json contains multiple top-level sections."
        ),
    )
    parser.add_argument(
        "--sort_keys",
    ...
    person_sub = load_person_subontology(person_json, root_key=args.root_key)
...
#NOTE: during mapping --> 'whether KNOWING THE STATE' of {person_domain} person domain would be relevant'
