...
"""
02_map_barriers_to_predictors.py  (dense predictor→barrier relevance)

Goal
----
For each PREDICTOR (aka "solution/intervention lever") leaf node, compute a DENSE relevance score (0..1000)
for EVERY HAPA BARRIER leaf node.

Interpretation (for barrier-targeted recommendation)
----------------------------------------------------
- Barrier domain: obstacle to behavior change / adherence (HAPA-aligned).
- Predictor domain: intervention lever / solution component that can reduce, bypass, or compensate for barriers.

Score(barrier_id) answers:
"If we deploy this PREDICTOR (solution), how strongly does it *directly help overcome or mitigate* this BARRIER,
either by targeting the mechanism, removing friction, adding supports, or compensating for deficits?"

Scoring guidance
----------------
- 1000 = highly direct/primary lever for this barrier (strong, specific, mechanistically aligned)
- 700–999 = strong relevance
- 400–699 = moderate relevance
- 1–399 = weak but non-zero relevance (indirect/secondary, or context-dependent)
- 0 = no plausible relation / not helpful for that barrier

Key requirements implemented
----------------------------
- Loops through ALL predictors (one LLM call per predictor leaf).
- Output is DENSE: must contain ALL barrier IDs for each predictor; missing IDs trigger retry and ultimately fail that predictor.
...
---------------------------------
- predictor_to_barrier_map.json          (predictor key -> dense barrier scores + stats)
- predictor_to_barrier_map_dense.csv     (predictors x barrier_ids; dense matrix)
- predictor_to_barrier_edges_long.csv    (tidy/long form; one row per predictor-barrier edge)

Usage examples
--------------
# Test mode (first N predictors):
python 02_map_barriers_to_predictors.py --test-mode --n-test 20

# Full run:
python 02_map_barriers_to_predictors.py --full-run
...
DEFAULT_PREDICTORS_LIST_TXT = (
    ...
    "ontology_mappings/PREDICTOR/barrier_to_predictor/input_lists/predictors_list.txt"
)
DEFAULT_BARRIERS_LIST_TXT = (
    "/Users/stijnvanseveren/PythonProjects/MASTERPROEF/utils/official/"
    "ontology_mappings/PREDICTOR/barrier_to_predictor/input_lists/barriers_list.txt"
)

# Output base (requested by user)
DEFAULT_OUT_BASE = (
    "/Users/stijnvanseveren/PythonProjects/MASTERPROEF/utils/official/"
    "ontology_mappings/PREDICTOR/barrier_to_predictor/results"
)
...
def build_system_prompt() -> str:
    return (
        "You are a clinical-ontology mapping engineer for a barrier-targeted digital intervention system.\n"
        "Task: given ONE solution/intervention lever for optimizing (non-)clinical mental health issues, score for all barrier domain ID's its likelihood/relevance of obstructing an effective execution of the solution domain.\n\n"

        "DEFINITIONS\n"
        "- Barrier domain: obstacle to behavior change/adherence (HAPA-aligned ; Health Action Process Approach).\n"
        "- Predictor domain: a solution/intervention lever, tool, service, or technique that can reduce barriers.\n\n"

        "RELEVANCE DEFINITION (THIS IS WHAT YOU SCORE)\n"
        "Score(barrier_id) quantifies: If this predictor/solution is recommended within a digital intervention and the person attempts to use it, how strongly would this barrier (if active or severe) hinder adoption, correct use, or sustained execution of the predictor’s intended behavior change? High scores indicate that this barrier is a likely binding obstacle that would block, disrupt, or substantially reduce the predictor’s real-world effectiveness; low scores indicate that the predictor largely bypasses or is robust to this barrier.\n\n"

        "High scores indicates that recommendation of prompted {solution/predictor} domain can effectively be obstructed by {barrier_id}.\n\n"

        "SCORING (DENSE)\n"
        "You MUST output a score for EVERY barrier ID.\n"
        "- 1000: primary/direct lever for that barrier.\n"
        "- 700–999: strong relevance.\n"
        "- 400–699: moderate relevance.\n"
        "- 1–399: weak but non-zero relevance.\n"
        "- 0: no plausible relation / not helpful.\n"
        "Use integer unit steps only.\n\n"
        
        "ENSURE that relevance scores are well-calibrated both in terms of ABSOLUTE score and RELATIVE score to others across all barriers.\n"

        "INTERNAL REASONING (DO NOT OUTPUT)\n"
        "1) Interpret the predictor: what concrete user actions, capacities, or resources it requires (e.g., attention, skills, planning, motivation, access).\n"
        "2) For each barrier, evaluate the hindrance pathway: if this barrier is elevated, how likely is it to prevent starting, following through, or maintaining use of the predictor, and how severe would that interference be?\n"
        "3) Use the full 0..1000 scale comparatively across barriers for this predictor, assigning very high scores only to barriers that directly interfere with the predictor’s core mechanism or required user actions, and assigning 0 only when no plausible hindrance pathway exists.\n\n"
...
def build_user_prompt(
    ...
    core = (
        "TARGET PREDICTOR (score barrier mitigation relevance for THIS predictor)\n"
        ...
        for attempt in range(1, max_retries + 1):
            try:
                req: Dict[str, Any] = dict(
                    ...
                    metadata={"predictor_key": predictor_key, "purpose": "predictor_to_barrier_dense_scoring"},
                )
...
def process_one_predictor(
    ...
    print(f"\n[START] {predictor_key} | predictor='{predictor.name}' | barriers={len(expected_barrier_ids)} | calls=1")
...
def write_dense_csv_from_results(
    ...
    headers = ["predictor_id"] + barrier_ids
...
def main() -> None:
    ...
    out_json = os.path.join(out_dir, "predictor_to_barrier_map.json")
    out_csv_dense = os.path.join(out_dir, "predictor_to_barrier_map_dense.csv")
    out_csv_long = os.path.join(out_dir, "predictor_to_barrier_edges_long.csv")
    ...
    barriers_raw_text = "" if compact_prompts else load_raw_text(args.barriers_list)
    ...
    print("\n=== WRITE CSVs ===")
    write_dense_csv_from_results(results=results, predictors=predictors, barriers=barriers, out_csv=out_csv_dense)
    write_long_edges_csv(results=results, predictors=predictors, barriers=barriers, out_csv=out_csv_long)
