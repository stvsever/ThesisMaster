#!/usr/bin/env python3
"""
Extract leaf-node paths from a nested PREDICTOR ontology JSON and write them to a .txt file.
...
# --- Paths you provided ---
PREDICTOR_ONTOLOGY = Path(
    "/Users/stijnvanseveren/PythonProjects/MASTERPROEF/SystemComponents/PHOENIX_ontology/"
    "separate/01_raw/PREDICTOR/steps/01_raw/aggregated/PREDICTOR_ontology.json"
)

OUTPUT_DIR = Path(
    "/Users/stijnvanseveren/PythonProjects/MASTERPROEF/utils/official/"
    "multi_dimensional_feasibility_evaluation/PREDICTORS/utils/input"
)

DELIM = " | "
OUTPUT_FILENAME = "PREDICTOR_leaf_paths.txt"
...
def main() -> None:
    if not PREDICTOR_ONTOLOGY.exists():
        raise FileNotFoundError(f"Ontology JSON not found: {PREDICTOR_ONTOLOGY}")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    out_path = OUTPUT_DIR / OUTPUT_FILENAME

    with PREDICTOR_ONTOLOGY.open("r", encoding="utf-8") as f:
        data = json.load(f)
