...
\hypersetup{
  ...
  linkcolor=black,
  urlcolor=black,
  pdftitle={PHOENIX evaluatiestudie - HCP-PRE-06 (C06)},
...
\fancyhead[R]{\small\color{SlateMid}\textbf{HCP-PRE-06}\quad C06}
...
\centering
{\large\bfseries\color{DarkBlue}Deelnemerscode: \texttt{HCP-PRE-06}}\\[0.5em]
...
\subsection*{Casus 1: 32-jarige marketingprofessional}

\begin{complaintbox}[title={Casus 1: 32-jarige marketingprofessional; duur: 2 jaar}]
\small ``Zo'n twee jaar geleden begon ik toenemende spanning te ervaren in werkgerelateerde situaties waarbij ik zichtbaar ben of beoordeeld word. Voor elke vergadering of presentatie bereid ik mij buitensporig lang voor -- soms meerdere uren -- uit angst niet competent over te komen. Na afloop piekeren mijn gedachten urenlang over wat ik verkeerd gezegd of gedaan heb. Ik vermijd inmiddels actief kansen waarbij ik mij moet presenteren: ik neem nauwelijks het woord in vergaderingen, wijs leiderschapsrollen af en aarzel bij elke professionele zichtbaarheidsmogelijkheid. Mijn loopbaan lijdt hieronder: ik heb een promotie afgeslagen omdat ik de extra zichtbaarheid niet aankon.''
\end{complaintbox}
...
\newpage
\subsection*{Casus 1: 32-jarige marketingprofessional -- Deel 2}

\begin{complaintbox}[title={Casus 1: verkorte klachtomschrijving}]
\small 32-jarige marketingprofessional; duur: 2 jaar. Prestatieangst op het werk, post-event ruminatie, excessieve voorbereiding en vermijding van professionele zichtbaarheid.
\end{complaintbox}

\begin{contextbox}
\small\textbf{Gestandaardiseerde symptomen uit Deel 1:}
\begin{itemize}[topsep=2pt,itemsep=1pt]
\item \textbf{S-1} Prestatieangst op het werk
\item \textbf{S-2} Post-event ruminatie
\item \textbf{S-3} Excessieve voorbereiding
\item \textbf{S-4} Vermijding van zichtbaarheid
\end{itemize}
\end{contextbox}
...
\newpage
\subsection*{Casus 1: 32-jarige marketingprofessional -- Deel 3}

\begin{complaintbox}[title={Casus 1: 32-jarige marketingprofessional}]
\small Prestatieangst op het werk, post-event ruminatie, excessieve voorbereiding en vermijding van professionele zichtbaarheid. Duur: 2 jaar.
\end{complaintbox}

\begin{monitorbox}
\small\textbf{21-daagse monitoring:} Voorbereidingstijd per werkmoment (vergadering, presentatie): gem.\ 3,4 uur/dag. Post-event ruminatieduur na werksituaties: gem.\ 1,8 uur/dag. Vermijding van zichtbaarheid in werk: gem.\ 2,1 keer/dag. Professionele exposurestappen uitgevoerd: gem.\ 0,2/dag. Zelfevaluatiescore na sociale werksituaties: gem.\ 2,4/10.
\end{monitorbox}

\begin{center}
\begin{tikzpicture}[node distance=0pt]
  \node[prnode] (p1) at (0, 2.2) {BO-1\\Professionele exposure};
  \node[prnode] (p2) at (0, 1.1) {BO-2\\Post-event verwerking};
  \node[prnode] (p3) at (0, 0.0) {BO-3\\Zelfevaluatiebias};
  \node[prnode] (p4) at (0,-1.1) {BO-4\\Voorbereidingstijd};
  \node[prnode] (p5) at (0,-2.2) {BO-5\\Vermijdingslogging};
  \node[crnode] (cr1) at (9, 1.65) {S-1\\Prestatieangst op het werk};
  \node[crnode] (cr2) at (9, 0.55) {S-2\\Post-event ruminatie};
  \node[crnode] (cr3) at (9,-0.55) {S-3\\Excessieve voorbereiding};
  \node[crnode] (cr4) at (9,-1.65) {S-4\\Vermijding van zichtbaarheid};
  \draw[line width=4.7pt, draw=StrongRed!85, opacity=0.90] (p1.east) -- (cr1.west);
  \draw[line width=4.3pt, draw=StrongRed!85, opacity=0.90] (p1.east) -- (cr4.west);
  \draw[line width=4.5pt, draw=StrongRed!85, opacity=0.90] (p2.east) -- (cr2.west);
  \draw[line width=2.8pt, draw=StrongRed!85, opacity=0.90] (p2.east) -- (cr1.west);
  \draw[line width=4.0pt, draw=PrimaryBlue!80, opacity=0.90] (p3.east) -- (cr1.west);
  \draw[line width=3.5pt, draw=PrimaryBlue!80, opacity=0.90] (p3.east) -- (cr2.west);
  \draw[line width=5.0pt, draw=PrimaryBlue!80, opacity=0.90] (p4.east) -- (cr3.west);
  \draw[line width=0.45pt, draw=PrimaryBlue!80, opacity=0.90] (p4.east) -- (cr1.west);
  \draw[line width=3.8pt, draw=StrongRed!85, opacity=0.90] (p5.east) -- (cr4.west);
...
\textbf{BO-1: Professionele exposure},\quad
\textbf{BO-2: Post-event verwerking},\quad
\textbf{BO-3: Zelfevaluatiebias},\quad
\textbf{BO-4: Voorbereidingstijd},\quad
\textbf{BO-5: Vermijdingslogging}
...
\newpage
\subsection*{Casus 1: 32-jarige marketingprofessional -- Deel 4}

\begin{contextbox}
\small\textbf{Gestandaardiseerde behandeldoelen (abstract) voor Deel 4:}
\begin{enumerate}[topsep=2pt,itemsep=1pt,label=\arabic*.]
\item \textbf{Professionele exposureoefening}
\item \textbf{Ruminatiebeheersing}
\item \textbf{Vermijdingsbewustwording}
\end{enumerate}
\end{contextbox}
...
\checkitemBFS{2}{Presentatie, vergadering of zichtbaar werkmoment bewust opgezocht of deelgenomen (ja/nee)}
\checkitemBFS{3}{Medicatie op voorgeschreven tijdstip ingenomen (ja/nee)}
\checkitemBFS{4}{Alcoholconsumptie vanavond (eenheden)}
\checkitemBFS{5}{Exposurestap in werksituatie volledig doorlopen zonder vroegtijdig terugtrekken (ja/nee)}
\checkitemBFS{6}{Gebalanceerde maaltijden op vaste tijden genuttigd vandaag (ja/nee)}
\checkitemBFS{7}{Post-event ruminatie bewust afgesloten via schrijfoefening of afsluitritueel (ja/nee)}
...
\checkitemBFS{10}{Tijdsduur piekeren over werkprestatie bewust beperkt of omgeleid (min)}
\checkitemBFS{11}{Vast slaap- en waktijdstip aangehouden vandaag (ja/nee)}
\checkitemBFS{12}{Lichaamsbeweging vandaag (min)}
\checkitemBFS{13}{Vermijdingsepisode in werksituatie geregistreerd in dagboek (ja/nee)}
\checkitemBFS{14}{Cafeine-inname na 15.00 uur beperkt (ja/nee)}
\checkitemBFS{15}{Gesprek met vertrouwenspersoon of steunfiguur gevoerd vandaag (ja/nee)}
\checkitemBFS{16}{Aantal situaties waarbij zichtbaarheid of beoordeling bewust vermeden werd vandaag (aantal)}
...
\newpage
\subsection*{Casus 1: 32-jarige marketingprofessional -- Deel 5}
...
Primair probleem & Prestatieangst en post-event ruminatie leiden tot toenemende professionele vermijding en loopbaanschade. \\[0.15em]
Behandeldoel & Professionele exposure: bewust deelnemen aan beoordelbare werksituaties zonder terugtrekking of overmatige voorbereiding. \\[0.15em]
Voornaamste barriere & Negatieve uitkomstverwachting en zelfevaluatie: de persoon verwacht definitieve afwijzing of beoordeling als incompetent bij elk zichtbaar moment. \\[0.15em]
Copingstrategie & Gedragsexperiment met vooraf geformuleerde toetsingshypothese en een vaste tijdslimiet voor post-event evaluatie (max.\ 10 minuten). \\
\end{tabular}
\end{contextbox}
...
\begin{center}
\texttt{stijn.vanseveren@ugent.be}\quad met onderwerp:\quad \texttt{PHOENIX-PRE-HCP-PRE-06}
\end{center}
