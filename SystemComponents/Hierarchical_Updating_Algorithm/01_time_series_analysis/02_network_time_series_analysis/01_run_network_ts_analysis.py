...
def analyze_one_profile_from_readiness(
    ...
    n, p = X_obs.shape
    miss = float(np.mean(~np.isfinite(X_obs)))
    ...
    # ---- cluster heuristic #1 (error ratio)
    log("[tvKS] Cluster detection (error ratio stationary/tv)")
    cl = detect_tv_clusters_by_error_ratio(X_imp_z, t_norm, tv, st, ratio_threshold=float(CFG.ratio_threshold))
    ...
    # ---- cluster heuristic #2 (network distance change points)
    ...
    # ---- edge shifts (individual relationship shifts)
    ...
    # ---- predictions + metrics
    log("[metrics] One-step predictions + metrics")
    ...
    log("[importance] Predictor->criterion importance (tv + stationary)")
    if not pred_idx:
        pred_idx = list(range(p))  # fallback: all
    ...
    if crit_idx:
        ...
        pd.DataFrame().to_csv(net_out / "criterion_dependence_tv.csv", index=False)

    # ---- multicollinearity
    log("[multicollinearity] Computing VIF/condition/correlation flags")
    # Use lag matrix Z for predictor-side collinearity diagnostics
    Z = X_imp_z[:-1]
    ...
    # ---- network centrality/global metrics (temporal + stationary)
    log("[network metrics] Centrality + global metrics")
    if not HAVE_NETWORKX:
        log("  [WARN] networkx not available -> skipping centrality/global network metrics.")
    else:
        # temporal lagged network: Bhat(t)
        lagged_c_rows = []
        lagged_g_rows = []
        contemp_c_rows = []
        contemp_g_rows = []
        ...
        for ei in range(len(tv.estpoints)):
            ...
            cdf, cmeta = compute_node_centralities(Gd, directed=True)
            if not cdf.empty:
            ...
            # contemporaneous undirected: partial_corr(t)
            ...
            cdf2, cmeta2 = compute_node_centralities(Gu, directed=False)
            if not cdf2.empty:
        ...
        if lagged_c_rows:
            ...
            pd.DataFrame().to_csv(net_out / "temporal_lagged_node_centrality.csv", index=False)

        if contemp_c_rows:
            pd.concat(contemp_c_rows, ignore_index=True).to_csv(net_out / "temporal_contemp_node_centrality.csv", index=False)
        else:
            pd.DataFrame().to_csv(net_out / "temporal_contemp_node_centrality.csv", index=False)
        ...
        # stationary contemporaneous undirected
    ...
    m2_metrics = {"prediction": st_pred, "r2_node_mean": float(np.mean(st.r2_node)), "alphas": st.alphas.tolist()}
