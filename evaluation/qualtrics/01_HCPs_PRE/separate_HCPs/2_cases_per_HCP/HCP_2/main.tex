...
Geschatte duur & Ongeveer 45--55 minuten voor beide casussen samen (inclusief leestijd instructies en Deel~6) \\
\end{tabularx}
\end{tcolorbox}
\end{center}
...
EMA-metingen en (5) een mobiele coachingsboodschap. Aansluitend beantwoordt u in
\textbf{Deel 6} één algemene reflectievraag over uw digitale benaderingslogica.
...
sombere stemming). Symptomen zijn geen gedragingen of interventies.
...
geplande exposure, aerobe beweging). Een behandelingsoptie is \emph{nooit een symptoom zelf},
...
\item \textbf{Symptoom:} inslaapproblemen, anticipatieangst, anergie, piekeren voor het slapengaan.
\item \textbf{Behandelingsoptie:} avondschermtijd (min), geplande exposurestap (ja/nee),
...
\small
Na voldoende EMA-monitoring construeert PHOENIX een \textbf{bipartiet netwerk}:
...
6 & Reflectie digitale aanpak & Beantwoord één algemene reflectievraag over uw EMA-benaderingslogica & $\approx$\,5 min \\
\midrule
 & \textbf{Totaal} & & \textbf{45--55 min} \\
\bottomrule
\end{tabular}
\end{center}
...
\item \textbf{Werk sequentieel: Deel 1 $\rightarrow$ Deel 6.}
Ga pas naar een volgend deel wanneer het huidige deel volledig is afgewerkt. Voor Deel 1--5 geldt dit voor beide casussen; Deel 6 is éénmalig.
...
\textbf{Let op:} variabelen als ``eetlust'', ``spierspanning'' of ``moeheid'' zijn
...
\begin{itemize}[topsep=2pt,itemsep=1pt]
\item \textbf{S-1} Emotionele uitputting
\item \textbf{S-2} Professionele depersonalisatie
\item \textbf{S-3} Sociale terugtrekking
\item \textbf{S-4} Cognitieve fouten op het werk
\end{itemize}
\end{contextbox}
...
\small\textbf{Gestandaardiseerde symptomen uit Deel 1:}
\begin{itemize}[topsep=2pt,itemsep=1pt]
\item \textbf{S-1} Aanhoudende rouwreactie
\item \textbf{S-2} Doorslaapinsomnie
\item \textbf{S-3} Schuldgedreven ruminatie
\item \textbf{S-4} Verminderde toekomstorientatie
\end{itemize}
\end{contextbox}
...
\item \textbf{Blauwe rand:} positief verband (meer BO $\rightarrow$ meer S).
\item \textbf{Rode rand:} negatief verband (meer BO $\rightarrow$ minder S).
\item \textbf{Lijndikte:} proportioneel aan de sterkte van het verband (21-daagse EMA-data).
\end{itemize}

\textbf{Antwoordformat:} vul alle 5 prioriteitslijnen in, van rangorde 1 (hoogste) tot 5 (laagste).

\medskip
\textit{\textbf{Zie het uitgewerkte voorbeeld hieronder} voor een toelichting op de prioriteringslogica.}
\end{tcolorbox}
...
(gem.\ 65 min). Lichaamsbeweging: 0,8 sessies/week. Sociaal contact op eigen initiatief: \textbf{5 keer/week} (reeds adequaat).
...
\item \textbf{BO-1 (Avondschermgebruik)} -- raakt S-1 en S-2; monitoring toont 18/21 avonden hoog gebruik $\rightarrow$ \textbf{prioriteit 1}.
\item \textbf{BO-2 (Lichaamsbeweging)} -- beschermend voor S-1 en S-2; frequentie extreem laag $\rightarrow$ \textbf{prioriteit 2}.
\item \textbf{BO-3 (Sociaal contact)} -- sterk negatief verband met S-3; contact echter reeds frequent (\textbf{5 keer/week} -- toestand reeds adequaat) $\rightarrow$ laagste prioriteit $\rightarrow$ \textbf{prioriteit 3}.
\end{enumerate}
\end{voorbeeldbox}
...
\begin{center}
\begin{tikzpicture}[node distance=0pt]
  \node[prnode] (p1) at (0, 2.2) {BO-1\\Werk-privegrens};
  \node[prnode] (p2) at (0, 1.1) {BO-2\\Herstelactiviteit};
  \node[prnode] (p3) at (0, 0.0) {BO-3\\Sociaal initiatief};
  \node[prnode] (p4) at (0,-1.1) {BO-4\\Professionele meestervaring};
  \node[prnode] (p5) at (0,-2.2) {BO-5\\Slaapkwaliteit};
  \node[crnode] (cr1) at (9, 1.65) {S-1\\Emotionele uitputting};
  \node[crnode] (cr2) at (9, 0.55) {S-2\\Professionele depersonalisatie};
  \node[crnode] (cr3) at (9,-0.55) {S-3\\Sociale terugtrekking};
  \node[crnode] (cr4) at (9,-1.65) {S-4\\Cognitieve fouten op het werk};
...
\textbf{BO-1: Werk-privegrens},\quad
\textbf{BO-2: Herstelactiviteit},\quad
\textbf{BO-3: Sociaal initiatief},\quad
\textbf{BO-4: Professionele meestervaring},\quad
\textbf{BO-5: Slaapkwaliteit}
...
\begin{center}
\begin{tikzpicture}[node distance=0pt]
  \node[prnode] (p1) at (0, 2.2) {BO-1\\Betekenisgerichte activiteit};
  \node[prnode] (p2) at (0, 1.1) {BO-2\\Slaap-waakconsistentie};
  \node[prnode] (p3) at (0, 0.0) {BO-3\\Sociale steunopname};
  \node[prnode] (p4) at (0,-1.1) {BO-4\\Zelfcompassie};
  \node[prnode] (p5) at (0,-2.2) {BO-5\\Toekomstplanning};
  \node[crnode] (cr1) at (9, 1.65) {S-1\\Aanhoudende rouwreactie};
  \node[crnode] (cr2) at (9, 0.55) {S-2\\Doorslaapinsomnie};
  \node[crnode] (cr3) at (9,-0.55) {S-3\\Schuldgedreven ruminatie};
  \node[crnode] (cr4) at (9,-1.65) {S-4\\Verminderde toekomstorientatie};
...
\noindent\textbf{Beschikbare behandelingsopties:}\enspace
\textbf{BO-1: Betekenisgerichte activiteit},\quad
\textbf{BO-2: Slaap-waakconsistentie},\quad
\textbf{BO-3: Sociale steunopname},\quad
\textbf{BO-4: Zelfcompassie},\quad
\textbf{BO-5: Toekomstplanning}
...
\item Vermijd items die het domein slechts \emph{zijdelings} raken.
\end{itemize}

\textbf{Belangrijk:} alle 20 items zijn behandelingsoptie-type EMA-items
(modificeerbare gedragingen -- \emph{geen} symptomen).
...
Stel: doel 1 = \textit{Slaapkwaliteit}. In de lijst staan o.a.:
\begin{itemize}[topsep=2pt,itemsep=1pt]
\item \checkmark\ ``Schermvrij interval direct voor slapengaan (min)'' $\rightarrow$ \textbf{juist} (directe operationalisering)
\item \checkmark\ ``Vaste bedtijd aangehouden (ja/nee)'' $\rightarrow$ \textbf{juist} (complementaire meting: regelmaat)
\item $\times$\ ``Duur aerobe activiteit vandaag (min)'' $\rightarrow$ \textbf{niet juist} (operationaliseert lichamelijke activiteit, niet Slaapkwaliteit)
\item $\times$\ ``Maaltijd op vaste tijden (ja/nee)'' $\rightarrow$ \textbf{niet juist} (hoort bij Voedingskwaliteit, niet bij Slaapkwaliteit)
\end{itemize}
...
\begin{enumerate}[topsep=2pt,itemsep=1pt,label=\arabic*.]
\item \textbf{Mentale werkontkoppeling}
\item \textbf{Buitenwerkse herstelactiviteit}
\item \textbf{Professionele competentie-ervaring}
\end{enumerate}
\end{contextbox}
...
\checkitemBFS{1}{Bewust sociaal contact geinitieerd buiten werkverband (ja/nee)}
\checkitemBFS{2}{Werkmail of -berichten bewust niet gecheckt na werktijd (ja/nee)}
\checkitemBFS{3}{Bewust contact met collega of leerling buiten leslokaal gezocht (ja/nee)}
\checkitemBFS{4}{Werkgerelateerde gedachten actief afgesloten via vaste afsluiting na werk (ja/nee)}
\checkitemBFS{5}{Cafeine-inname na 15.00 uur (aantal)}
\checkitemBFS{6}{Maaltijd als zelfzorgmoment op vaste tijd genuttigd (ja/nee)}
...
\checkitemBFS{11}{Herstelactiviteit ingepland voor de volgende dag (ja/nee)}
\checkitemBFS{12}{Lichaamsbeweging als herstelactiviteit na werk (min)}
\checkitemBFS{13}{Alcoholinname vanavond (eenheden)}
\checkitemBFS{14}{Moment van professionele positieve prestatie bewust opgemerkt (ja/nee)}
\checkitemBFS{15}{Schermtijd in de late avond bewust beperkt (min)}
\checkitemBFS{16}{Aantal nieuwschecks in de avond beperkt (aantal)}
...
\small\textbf{Gestandaardiseerde behandeldoelen (abstract) voor Deel 4:}
\begin{enumerate}[topsep=2pt,itemsep=1pt,label=\arabic*.]
\item \textbf{Waardegerichte activiteit}
\item \textbf{Slaapregulariteit}
\item \textbf{Cognitieve herwaardering}
\end{enumerate}
\end{contextbox}
...
\checkitemBFS{3}{Activiteit die een persoonlijke waarde of herinnering belichaamt uitgevoerd (ja/nee)}
...
\checkitemBFS{9}{Ontspanningsoefening of rustige activiteit in de avond (min)}
\checkitemBFS{10}{Alcoholinname vanavond (eenheden)}
\checkitemBFS{11}{Afwijking van gewone slaap- of waktijdstip vandaag (min)}
...
\checkitemBFS{17}{Zelfcompassie-oefening bewust toegepast vandaag (min)}
\checkitemBFS{18}{Middagdutje genomen (ja/nee)}
\checkitemBFS{19}{Vrijwilligerswerk of helpende activiteit voor anderen (min)}
\checkitemBFS{20}{Dankbaarheidsnotitie of positief dagboek bijgehouden (ja/nee)}
\vspace{0.4em}
\noindent\textbf{Totaal geselecteerd:}\hspace{0.5em}\rule{1.2cm}{0.25pt}\hspace{0.2em}/ 6
\end{responsebox}
...
% ══════════════════════════════════════════════════════════════════════════════
\section{Deel 5: Gepersonaliseerde mobiele coachingsboodschap}
...
De boodschap erkent de barriere (gewoontekracht), geeft een concrete actie met tijdstip, verlaagt de drempel en vergroot de zelfeffectiviteit.
\end{voorbeeldbox}
...
% ══════════════════════════════════════════════════════════════════════════════
\section{Deel 6: Reflectie op digitale benaderingslogica}

\begin{tcolorbox}[colback=AccentOrange,colframe=GoldAmber,arc=2.5mm,boxrule=0.9pt,
  left=10pt,right=10pt,top=9pt,bottom=9pt]
\textbf{\color{GoldAmber}Instructies Deel 6 -- Algemene reflectievraag}

\smallskip\small
\textbf{Context -- de PHOENIX-benaderingslogica:}\\[0.3em]
PHOENIX verwerkt een vrije klachttekst via vijf opeenvolgende stappen:
(1)~identificatie van klacht- en toestandsdimensies als meetbare \emph{symptomen},
(2)~generatie van modificeerbare gedragsvariabelen (\emph{behandelingsopties}),
(3)~prioritering op basis van een bipartiet netwerk dat op dagelijkse EMA-data wordt gefit,
...
De kern van deze aanpak is \textbf{symptoomgedreven netwerk-analyse}: klachten worden niet als
uitingen van een vaste stoornis beschouwd, maar als dynamische knooppunten in een netwerk van
onderling samenhangende toestandsdimensies. Behandelprioriteiten worden \emph{niet}
klinisch verondersteld, maar \emph{empirisch} bepaald op basis van de gemeten verbanden
tussen dagelijks gedrag (behandelingsopties) en klachtniveaus (symptomen) over de tijd.

\medskip
\textbf{Hypothetische situatie:}\\[0.3em]
Stelt u zich voor dat u als zorgprofessional een vrije klachttekst ontvangt van een persoon
met psychische klachten. U beschikt \emph{uitsluitend} over twee middelen:
\begin{itemize}[topsep=2pt,itemsep=1pt]
\item een \textbf{mobiele applicatie} waarmee de persoon elke dag een korte set vragen
beantwoordt (bijv.\ ja/nee, 0--10 schaal, aantal, minuten) -- een EMA-design met
dagelijkse datacollectie,
\item een \textbf{dagelijks digitaal bericht} dat u -- of een geautomatiseerd systeem --
naar de persoon stuurt (zoals u in Deel 5 formuleerde).
\end{itemize}
U heeft \emph{geen} directe consulten, telefoongesprekken of andere contactvormen ter beschikking.
\emph{Alleen} dagelijkse datacollectie via de app en een dagelijkse geschreven boodschap.

\medskip
\textbf{Reflectievraag:}\\[0.3em]
Hoe zou u, als zorgprofessional, te werk gaan in deze situatie?
Beschrijf zo concreet mogelijk uw benaderingslogica:
\begin{enumerate}[topsep=2pt,itemsep=1pt,label=\alph*.]
\item Welke informatie zou u via de dagelijkse EMA-vragen willen verzamelen, en hoe zou u
die selectie bepalen op basis van de vrije klachttekst?
\item Hoe zou u de dagelijkse boodschappen inhoudelijk afstemmen op wat u via de
EMA-data geleidelijk leert over de toestand en het gedrag van de persoon?
\item Zou u uw digitale aanpak systematisch bijsturen naarmate meer data beschikbaar wordt?
Zo ja -- op basis van welke criteria en via welk redeneerproces?
\end{enumerate}

\medskip\small
\textit{Er is geen enkel correct antwoord. Dit deel dient uitsluitend om uw klinische
benaderingslogica in kaart te brengen, zodat die naast die van PHOENIX kan worden
geplaatst. Neem voldoende ruimte om uw redenering volledig en concreet te beschrijven.}
\end{tcolorbox}

\vspace{0.5em}

\begin{responsebox}
\small\textbf{Uw antwoord:}
\textit{Beschrijf uw aanpak zo concreet mogelijk -- denk aan: welke vragen, welke criteria,
hoe u de boodschap aanpast op basis van data, en hoe uw redeneerproces eruitziet.}

\writeline\writeline\writeline\writeline\writeline\writeline\writeline\writeline\writeline\writeline\writeline\writeline\writeline\writeline\writeline\writeline
\end{responsebox}
...
Dank u voor het invullen van alle zes delen voor uw twee toegewezen casussen
(Casus 1 en Casus 2) en de afsluitende reflectievraag.
...
\item[$\square$] Ik heb in Deel 6 de reflectievraag over digitale benaderingslogica beantwoord.
