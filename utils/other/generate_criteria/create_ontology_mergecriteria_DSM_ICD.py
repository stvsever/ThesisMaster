...
def process_hierarchy(g, hierarchy, parent_labels=None, path=None, class_depths=None):
    ...
    - Merges generated criteria by creating child classes for any 'criteria' lists.
    - Ignores prevalence fields.
    - Also supports nodes whose value is directly a list (treated as criteria).
    """
    if parent_labels is None:
        parent_labels = []
    if path is None:
    ...
    for label, subclasses in hierarchy.items():
        ...
        # If this node is a dict, handle 'criteria' and recurse into other keys
        if isinstance(subclasses, dict):
            # 1) Merge criteria under this node (if present)
            if "criteria" in subclasses and isinstance(subclasses["criteria"], list):
                for crit in subclasses["criteria"]:
            ...
            # 2) Recurse into other keys (excluding prevalence/metadata)
            for k, v in subclasses.items():
                if k in {"criteria", "estimated_prevalence_percent", "prevalence", "prevalence_percent"}:
                    continue
                if isinstance(v, (dict, list)):
                    # Wrap as a mini-hierarchy so our function signature stays consistent
                    process_hierarchy(
                        g,
                        {k: v},
                        parent_labels=current_path,
        ...
        # If this node is directly a list, treat it as a criteria list
        elif isinstance(subclasses, list):
            for crit in subclasses:
                if isinstance(crit, str) and crit.strip():
                    add_class(
                        g,
                        crit,
        ...
        # Other types (str/number/None) are ignored
...
def main():
    # Configuration
    input_json_path = "/SystemComponents/PHOENIX_ontology/separate/CRITERION/steps/01_raw/clinical/02_post_generation/ICD_pg/ICD_criteria_merged.json"  # Single json file
    output_dir = "/SystemComponents/PHOENIX_ontology/separate/CRITERION/steps/01_raw/clinical/02_post_generation/ICD_pg"  # Output directory
    ...
    add_ontology_metadata(
        g,
        title="PHOENIX PHOENIX_ontology - subset: Clinical ICD-10 based ontology with generated criteria",
        iri=BASE_IRI,
