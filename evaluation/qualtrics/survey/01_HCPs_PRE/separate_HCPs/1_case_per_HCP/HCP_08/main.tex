...
\hypersetup{
  ...
  linkcolor=black,
  urlcolor=black,
  pdftitle={PHOENIX evaluatiestudie - HCP-PRE-08 (C08)},
...
\fancyhead[R]{\small\color{SlateMid}\textbf{HCP-PRE-08}\quad C08}
...
\centering
{\large\bfseries\color{DarkBlue}Deelnemerscode: \texttt{HCP-PRE-08}}\\[0.5em]
...
\subsection*{Casus 1: 38-jarige projectmanager}

\begin{complaintbox}[title={Casus 1: 38-jarige projectmanager; duur: sinds de kindertijd, duidelijk verergerd in de voorbije 12 maanden}]
...
\newpage
\subsection*{Casus 1: 38-jarige projectmanager -- Deel 2}
...
\newpage
\subsection*{Casus 1: 38-jarige projectmanager -- Deel 3}

\begin{complaintbox}[title={Casus 1: 38-jarige projectmanager}]
...
\begin{tikzpicture}[node distance=0pt]
  ...
  \draw[line width=0.45pt, draw=StrongRed!85, opacity=0.90] (bo2.east) -- (s4.west);
  \draw[line width=0.45pt, draw=StrongRed!85, opacity=0.90] (bo2.east) -- (s1.west);
...
\newpage
\subsection*{Casus 1: 38-jarige projectmanager -- Deel 4}

\begin{contextbox}
\small\textbf{Gestandaardiseerde behandeldoelen (abstract) voor Deel 4:}
\begin{enumerate}[topsep=2pt,itemsep=1pt,label=\arabic*.]
\item \textbf{Externe taakstructuur}
\item \textbf{Aandachtsregulatie in gesprek}
\item \textbf{Positieve bekrachtiging bij voltooiing}
\end{enumerate}
\end{contextbox}
...
\checkitemBFS{1}{Cafeine-inname voor de middag beperkt (ja/nee)}
...
\checkitemBFS{10}{Alcoholconsumptie vanavond (eenheden)}
...
\checkitemBFS{13}{Financiele zorgen of administratieve taken aangepakt vandaag (ja/nee)}
\checkitemBFS{14}{Lichttherapielamp of wekker op licht gebruikt in de ochtend (ja/nee)}
\checkitemBFS{15}{Kleine symbolische beloning gegeven na taakvoltooiing (ja/nee)}
\checkitemBFS{16}{Vast slaap- en waktijdstip aangehouden vandaag (ja/nee)}
\checkitemBFS{17}{Gesprek met vertrouwenspersoon of steunfiguur gevoerd vandaag (ja/nee)}
\checkitemBFS{18}{Lichaamsbeweging of wandeling buiten uitgevoerd (min)}
...
\newpage
\subsection*{Casus 1: 38-jarige projectmanager -- Deel 5}
...
\begin{center}
\texttt{stijn.vanseveren@ugent.be}\quad met onderwerp:\quad \texttt{PHOENIX-PRE-HCP-PRE-08}
\end{center}
