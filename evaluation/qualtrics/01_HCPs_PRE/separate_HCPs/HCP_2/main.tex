...
behandelingsoptie actief bijdraagt aan de klacht?
...
\item \textbf{BO-3 (Sociaal contact)} -- sterk negatief verband met S-3; laag (0,9/week) $\rightarrow$ \textbf{prioriteit 3}.
\end{enumerate}
\end{voorbeeldbox}
...
(bijv.\ ``herstel en ontspanning'' of ``slaapregulariteit'').
...
\begin{itemize}[topsep=2pt,itemsep=1pt]
\item Een abstract behandeldoel beschrijft een \emph{gedragsdomein} (bijv.\ werkgrenzen, herstel, waardegerichte activiteit).
\item Een sub-behandelingsoptie is een \emph{specifiek, dagelijks meetbaar gedragsitem} dat dat domein het directst operationaliseert.
...
Stel: doel 1 = \textit{Slaap en avondontspanning}. In de lijst staan o.a.:
\begin{itemize}[topsep=2pt,itemsep=1pt]
...
\item $\times$\ ``Cafeïne-inname na 15.00 uur (aantal)'' $\rightarrow$ \textbf{niet juist} (zijdelings)
\item $\times$\ ``Maaltijd op vaste tijden (ja/nee)'' $\rightarrow$ \textbf{niet juist} (ander domein)
\end{itemize}
...
\begin{enumerate}[topsep=2pt,itemsep=1pt,label=\arabic*.]
\item \textbf{Werkgrenzen en mentale ontkoppeling}\quad\textit{\small (domein: stoppen met werk na werktijd; frequente overschrijding (5,4 episodes/dag))}
\item \textbf{Herstel en ontspanning buiten werk}\quad\textit{\small (domein: actief herstelgedrag na werk; kritisch laag niveau (1,2/week))}
\item \textbf{Professionele competentie-ervaring}\quad\textit{\small (domein: bewust opmerken en benutten van professionele succesmomenten; kan depersonalisatie temperen)}
\end{enumerate}
\end{contextbox}
...
\small\textbf{Gestandaardiseerde behandeldoelen (abstract) voor Deel 4:}
\begin{enumerate}[topsep=2pt,itemsep=1pt,label=\arabic*.]
\item \textbf{Betekenis en waardegerichte activiteit}\quad\textit{\small (domein: activiteiten die persoonlijke waarden of herinneringen belichamen; slechts 4/21 dagen uitgevoerd)}
\item \textbf{Slaapregulariteit}\quad\textit{\small (domein: consistent slaap-waakritme aanhouden; lage slaapkwaliteit (2,8/10) ondermijnt emotieregulatie)}
\item \textbf{Zelfcompassie en cognitieve herwaardering}\quad\textit{\small (domein: zelfkritische gedachten omvormen; schuldgedreven ruminatie is hoog (5,8/dag))}
\end{enumerate}
\end{contextbox}
